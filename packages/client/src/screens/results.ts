...
import type { TeamSummary } from './game';
...
export function ctfHeadline(summary: TeamSummary): string {
  const w = matchWinner(summary.redScore, summary.blueScore);
  const score = `${summary.redScore} \u2013 ${summary.blueScore}`;
...
export function ctfOutcome(summary: TeamSummary, myTeam: number | undefined): CtfOutcome | undefined {
...
/** Two ranked team panels; td has no captures, so no Caps column. */
function teamBody(ranking: RankRow[], meId: string, summary: TeamSummary): { title: string; titleClass: string; sub: string; body: string } {
  const me = ranking.find((r) => r.id === meId);
  const outcome = ctfOutcome(summary, me?.team);
  const winner = matchWinner(summary.redScore, summary.blueScore);
  const caps = summary.mode !== 'td';
  ...
  return {
    ...
    titleClass: outcome ?? '',
    sub: outcome
      ? `<span class="t-red side-score">Red ${summary.redScore}</span> \u2013 <span class="t-blue side-score">${summary.blueScore} Blue</span>`
      : escapeHtml(ctfHeadline(summary)),
    body: `<div class="teams">${side(TEAM_RED, red, summary.redScore)}${side(TEAM_BLUE, blue, summary.blueScore)}</div>`,
  };
}
...
export function showResults(
  ...
  root.className = team ? 'results ctf' : 'results';
  let title = 'Match over';
  let titleClass = '';
  let sub: string;
  let body: string;
  if (team) {
    ({ title, titleClass, sub, body } = teamBody(ranking, meId, team));
  } else {
    const medals = ['🥇', '🥈', '🥉'];
    const rows = ranking
